...
export default class PreseEveResult extends Component {
...
handleSearchArea=(e)=>{
  ...
  render() {
    return (
        ...
        <div>
          <center>
          <h1 style={{background:'#000080', width:"800px", color:'white'}}> Presentation Evaluvation Results</h1>
          <br></br><br></br>
          </center>  
          ...
          <table className='table' style={{width:'1000px', marginLeft:"250px"}}>
            ...
            {this.state.GetAllposts.map((GetAllposts,index)=>(
                <tr key ={index}>
                  ...
                  <td><a href={`/PresEveOneResult/${GetAllposts._id}`} style={{TextDecoration:'none'}}>{GetAllposts.Groupname} </a> </td>  
